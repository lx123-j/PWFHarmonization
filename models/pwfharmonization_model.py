...
# 定义生成器
    ...
    if self.isTrain:
      ...
      self.netD_B = networks.define_D(opt.input_nc, opt.ndf,
                                      opt.which_model_netD,
                                      opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids)
    if not self.isTrain or opt.continue_train:     # test或者继续训练时会把原本的model导出来使用
      which_epoch = opt.which_epoch
      self.load_network(self.netG_A, 'G_A', which_epoch)
      self.load_network(self.netG_B, 'G_B', which_epoch)
      if self.isTrain:
        self.load_network(self.netD_A, 'D_A', which_epoch)      # continue_train时
        self.load_network(self.netD_B, 'D_B', which_epoch)      # 也就是说在test的时候不需要载入G_A和G_B
  ...
  def save_images(self, save_path,image):
  ...
  def GeneratePseudoWarpingField(self, height, width, motion_level, shift_level, scale_level):
    # adapted from https://github.com/daooshee/ReReVST-Code/blob/master/train/loss_networks.py
    ''' height: img.shape[0]
        width:  img.shape[1] '''
    if scale_level > 0:   # 需要进行放缩操作
    ...
    if motion_level > 0:   # 只有这一部分是PseudoWarpingField生成   主要是为模拟图像中运动的效果，以用于训练和数据增强，使模型能更好处理运动信息
  ...
  def GenerateFakeData(self, first_frame, hyperparameters):      # 这里是GeneratePseudoWarpingField和新的图片
    # adapted from https://github.com/daooshee/ReReVST-Code/blob/master/train/loss_networks.py
    B, C, H, W = first_frame.size()

    fake_flow = self.GeneratePseudoWarpingField(H, W, hyperparameters.motion_level, hyperparameters.shift_level, hyperparameters.scale_level)

    if first_frame.is_cuda:
        fake_flow = fake_flow.cuda()
    fake_flow = fake_flow.expand(B, 2, H, W)
    second_frame = util.warp(first_frame, fake_flow)   # 生成下一帧
    # 消融实验
    # second_frame = util.warp(first_frame)  # 生成下一帧
  ...
  def set_input(self, input):
    ...
    input_A = input['A1']     # 这里的A1和B1在 ADNI.py 那里 是两张不同的输入图片
    input_B = input['B1']     # 当实例对象通过[] 运算符取值时，会调用它的方法__getitem__     同样是ADNI.py
  ...
  def backward_G(self):
    ...
    if self.opt.lambda_unsup_cycle_A > 0:
      ...
      #消融实验
      # real_A_warped = util.warp(self.real_A)
    ...
    if self.opt.lambda_unsup_cycle_B > 0:
      ...
      # 消融实验
      # real_B_warped = util.warp(self.real_B)
    ...
    if self.opt.lambda_spa_unsup_A > 0:
      ...
      # 消融实验
      # fake_B_warped = util.warp(self.fake_B)
    ...
    if self.opt.lambda_spa_unsup_B > 0:
      ...
      # 消融实验
      # fake_A_warped = util.warp(self.fake_A)
  ...
  def get_current_visuals(self):      # 这里是数据的显示
